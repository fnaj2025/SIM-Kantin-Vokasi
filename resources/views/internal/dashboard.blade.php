...
<div class="stats-grid">
    ...
    <div class="stat-card">
        <div class="stat-icon" style="background:{{ $s['icon_bg'] }}; color:{{ $s['icon_color'] }}">
            {!! $s['icon'] !!}
        </div>
        <div>
            <div class="stat-value">
                @if($s['format']==='currency')Rp {{ number_format($s['value'],0,',','.') }}
                @else{{ $s['value'] }}
                @endif
            </div>
    ...
    <div style="display:flex;flex-direction:column;gap:1.5rem">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <span style="font-size:.8rem;color:#64748B;font-weight:700">{{ now()->format('F Y') }}</span>
            </div>
            <div class="card-body">
                @php $maxRevenue = max(array_column($revenueChart, 'revenue') ?: [1]); @endphp
                <div class="chart-container">
                    @foreach($revenueChart as $point)
                    @php $h = $maxRevenue > 0 ? max(8, round($point['revenue'] / $maxRevenue * 160)) : 8; @endphp
                    <div class="chart-bar-group" title="Rp {{ number_format($point['revenue'],0,',','.') }} — {{ $point['orders'] }} order">
                        <div class="chart-bar-main" style="height:{{ $h }}px"></div>
                ...
                <div style="display:flex;justify-content:space-between;margin-top:1.5rem;font-size:.75rem;color:#94A3B8;font-weight:700">
                    @foreach($revenueChart as $p)
                    <span>{{ $p['orders'] }} ord</span>
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('internal.orders.index') }}" style="font-size:.8rem;color:#EA580C;text-decoration:none;font-weight:800;display:flex;align-items:center;gap:0.25rem">
                    LIHAT SEMUA
            ...
            <div class="card-body" style="padding:0">
                <table class="order-table">
                    ...
                    <tbody>
                        ...
                        @endphp
                        <tr>
                            <td><span style="font-weight:800;color:#0F172A">{{ $order->order_number }}</span></td>
                            <td style="font-weight:500">{{ $order->customer_name }}</td>
                            <td><span style="font-weight:800">Rp {{ number_format($order->total,0,',','.') }}</span></td>
                            <td><span class="status-pill" style="background:{{ $statusColor[0] }};color:{{ $statusColor[1] }}">{{ $order->status }}</span></td>
                            <td style="color:#94A3B8;font-weight:600">{{ $order->created_at->format('H:i') }}</td>
                        </tr>
                        @empty
    ...
    <div style="display:flex;flex-direction:column;gap:1.5rem">
        ...
        <div class="ai-panel">
            ...
            @if(count($aiAlerts) > 0)
                ...
                <div class="ai-alert {{ $alert['level'] }}">
            ...
            @if(count($aiInsights) > 0)
                ...
                <div class="ai-alert {{ $insight['type'] }}">
            ...
            @if(count($aiAlerts) === 0 && count($aiInsights) === 0)
            <div class="ai-empty">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('internal.kitchen') }}" style="font-size:.8rem;color:#EA580C;text-decoration:none;font-weight:800">KDS →</a>
            </div>
            <div class="card-body">
                <div class="mini-grid">
                    <div class="mini-card" style="border-left:3px solid #D97706">
                        <div class="mini-label">Menunggu</div>
                        <div class="mini-value" style="color:#D97706">{{ $kitchenAgent['pending'] ?? 0 }}</div>
                    </div>
                    <div class="mini-card" style="border-left:3px solid #2563EB">
                        <div class="mini-label">Dimasak</div>
                        <div class="mini-value" style="color:#2563EB">{{ $kitchenAgent['in_progress'] ?? 0 }}</div>
                    </div>
                    <div class="mini-card" style="border-left:3px solid #059669">
                        <div class="mini-label">Siap</div>
                        <div class="mini-value" style="color:#059669">{{ $kitchenAgent['ready'] ?? 0 }}</div>
                    </div>
                    <div class="mini-card" style="border-left:3px solid #DC2626">
                        <div class="mini-label">Late</div>
                        <div class="mini-value" style="color:#DC2626">{{ $kitchenAgent['overdue_count'] ?? 0 }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <a href="{{ route('internal.finance.index') }}" style="font-size:.8rem;color:#EA580C;text-decoration:none;font-weight:800">DETAIL →</a>
            </div>
            <div class="card-body">
                <div style="display:flex;flex-direction:column;gap:1rem">
                    <div style="display:flex;justify-content:space-between;align-items:center">
                        <span style="font-size:.875rem;color:#64748B;font-weight:600">Pemasukan</span>
                        <span style="font-weight:800;color:#16A34A">Rp {{ number_format($financeAgent['income_today']??0,0,',','.') }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between;align-items:center">
                        <span style="font-size:.875rem;color:#64748B;font-weight:600">Pengeluaran</span>
                        <span style="font-weight:800;color:#DC2626">Rp {{ number_format($financeAgent['expense_today']??0,0,',','.') }}</span>
                    </div>
                    <div style="border-top:1px dashed #E2E8F0;padding-top:1rem;display:flex;justify-content:space-between;align-items:center">
                        ...
                        <span style="font-weight:900;font-size:1.1rem;color:{{ ($financeAgent['balance_today']??0) >= 0 ? '#0F172A' : '#DC2626' }}">
                            Rp {{ number_format($financeAgent['balance_today']??0,0,',','.') }}
                        </span>
                    </div>
                </div>
            </div>
...
{{-- Top Menus --}}
@if(!empty($analyticsAgent['top_menus']))
<div class="card" style="margin-top:1.5rem">
    ...
    <div class="card-body">
        <div class="top-menu-grid">
            @foreach($analyticsAgent['top_menus'] as $i => $menu)
            <div class="top-menu-item">
                <div class="top-menu-rank">{{ $i + 1 }}</div>
                <div style="font-weight:800;font-size:.875rem;color:#0F172A;margin-bottom:.5rem">{{ $menu['name'] }}</div>
                <div style="font-size:.8rem;color:#EA580C;font-weight:800;margin-bottom:0.25rem">{{ $menu['qty'] }} porsi</div>
                <div style="font-size:.75rem;color:#94A3B8;font-weight:600">Rp {{ number_format($menu['revenue'],0,',','.') }}</div>
            </div>
            @endforeach
        </div>
    </div>
